...
const CFG = {
    ...
    BASE_ESTIMATED_COST: 0.012, // Basisgebühren ohne Spende
    TOKEN_STANDARD: 4
};
...
import { setComputeUnitLimit, setComputeUnitPrice, transferSol } from "https://esm.sh/@metaplex-foundation/mpl-toolbox@0.10.0";
...
const httpForId = (id, gatewayIndex = 0) => {
    const gateway = CFG.GATEWAYS[gatewayIndex];
    return `${gateway}/${CFG.JSON_BASE_CID}/${id}.json`;
};
...
function getSelectedDonation() {
    ...
    if (selected.value === 'custom') {
        const customValue = parseFloat($("customDonationInput").value);
...
async function loadClaims() {
    try {
        const gateway = CFG.GATEWAYS[0];
        const url = `${gateway}/${CFG.CLAIMS_CID}`;
        ...
        for (let i = 0; i < CFG.GATEWAYS.length; i++) {
            try {
                ...
                if (Array.isArray(meta.attributes)) {
                    add("Attribute", meta.attributes.map(a => `${a.trait_type}: ${a.value}`).join(" · "));
                }
...
async function doMint() {
    try {
        ...
        if (!Number.isInteger(id) || id < 0 || id > CFG.MAX_INDEX) {
            throw new Error(`Ungültige Token-Nummer: 0-${CFG.MAX_INDEX}`);
        }
        ...
        if (solBalance < totalCost) {
            throw new Error(`Unzureichendes Guthaben. Benötigt: ${totalCost.toFixed(3)} SOL`);
        }
...
function handleError(context, e) {
    ...
    if (/user rejected/i.test(errorMsg)) {
        userMsg = "Du hast die Transaktion abgelehnt";
    }
    else if (/insufficient funds/i.test(errorMsg)) {
        userMsg = "Unzureichendes SOL-Guthaben";
    }
    else if (/already in use/i.test(errorMsg)) {
        userMsg = "Diese ID wurde bereits gemintet";
        setRandomFreeId();
    }
