...
/* ========== Thin compatibility wrappers (v2/v3) ========== */
function tmCreateMetadataInstr(accounts, dataV2Like) {
  const fn = TM.createCreateMetadataAccountV2Instruction || TM.createCreateMetadataAccountV3Instruction;
  if (!fn) throw new Error("mpl-token-metadata: CreateMetadata (v2/v3) nicht verfügbar");

  const arg = fn.name.includes("V3")
    ? { createMetadataAccountArgsV3: { data: dataV2Like, isMutable: true, collectionDetails: null } }
    : { createMetadataAccountArgsV2: { data: dataV2Like, isMutable: true } };
...
function tmVerifyCollectionInstr(obj) {
  const fn = TM.createSetAndVerifyCollectionInstruction || TM.createVerifyCollectionInstruction;
  return fn ? fn(obj) : null;
}
...
/* ==================== IPFS Helpers ==================== */
const fetchWithTimeout = (u, ms = 8000) => Promise.race([
...
async function updatePreview() {
  ...
  try {
    ...
    log("Alle Gateways fehlgeschlagen für ID " + id, e);
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  log("System boot", {
    build: BUILD_TAG, rpcs: CFG.RPCS, roles: {
      creator: CFG.CREATOR,
